...
const MODULE_NAME = 'OCPIPullTariffsTask';

export default class OCPIPullTariffsTask extends TenantSchedulerTask {
  ...
  private async processOCPIEndpoint(tenant: Tenant, ocpiEndpoint: OCPIEndpoint): Promise<void> {
    // Get the lock
    const ocpiLock = await LockingHelper.createOCPIPullTariffsLock(tenant.id, ocpiEndpoint);
    if (ocpiLock) {
      try {
        ...
        await Logging.logInfo({
          ...
          module: MODULE_NAME, method: 'processOCPIEndpoint',
          action: ServerAction.OCPI_EMSP_GET_SESSION,
          message: `Pull of Tariffs for endpoint '${ocpiEndpoint.name}' is being processed`
        });
        // Build OCPI Client
        const ocpiClient = await OCPIClientFactory.getEmspOcpiClient(tenant, ocpiEndpoint);
        // Pull Tariffs
        const result = await ocpiClient.pullTariffs(true);
        await Logging.logInfo({
          tenantID: tenant.id,
          module: MODULE_NAME, method: 'processOCPIEndpoint',
          action: ServerAction.OCPI_EMSP_GET_SESSION,
          message: `Pull of Tariffs for endpoint '${ocpiEndpoint.name}' is completed`,
